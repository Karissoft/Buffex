    ...
    <div class="grid grid-cols-3 gap-6">
              ...
              >
                <table class="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody class="bg-white divide-y divide-gray-200">
                    <tr v-for="orders in filteredorders" :key="orders.id" class="cursor-pointer" :class="orders.order_no==orderNo?'bg-purple-100':''" @click="handleHistory(orders)">
                      <td class="px-3 py-4 whitespace-nowrap">
                       ...
                       <td class="px-2 py-4">
                        <div class="text-sm text-gray-900">
                          <p class="capitalize">{{orders.user.name }}</p>
                           <p class="capitalize text-xs">{{orders.user.phone }}</p>
                            <p class=" text-xs">{{orders.user.email }}</p>
                        </div>
                      </td>
                    </tr>
                  </tbody>
      ...
      <div class="col-span-2">
        <div class="grid grid-cols-1 gap-6 mb-4">
          <div
            class="flex justify-between h-[40px] "
          >
          <h2 class="font-bold">{{orderNo}}</h2>
        <span class="flex items-center">
            <h2 class="font-bold">{{currency(total)}}</h2>
            <span v-if="status=='paid'" class="text-sm ml-3 bg-green-100 text-green-600 rounded-full px-3 py-1 capitalize">{{status}}</span>
             <span  v-if="status=='failed'" class="text-sm ml-3 bg-red-100 text-red-600 rounded-full px-3 py-1 capitalize">{{status}}</span>
              <span  v-if="status=='pending'" class="text-sm ml-3 bg-yellow-100 text-yellow-600 rounded-full px-3 py-1 capitalize">{{status}}</span>
        </span>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody class="bg-white divide-y divide-gray-200">
                    <tr v-for="history in histories" :key="history.id" >
                      ...
                      <td class="px-2 py-4">
                        <div class="text-sm text-gray-900">
                          <p class="line-clamp-2">{{ currency(history.price) }}</p>
                        </div>
                      </td>
                      <td class="px-2 py-4 whitespace-nowrap">
                        <span
                      ...
                      <td class="px-2 py-4 whitespace-nowrap">
                        <span
                          ...
                          "
                        >
                          {{ currency(history.price * history.quantity) }}
                        </span>
                      </td>
