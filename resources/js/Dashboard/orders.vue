                ...
                >
                    ...
                    >
                        ...
                        >
                            <button
                                type="button"
                                class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-800 sm:ml-3 sm:w-auto sm:text-sm"
                                @click="open = false"
                            >
                                Deactivate
                            </button>
                            <button
                                type="button"
                                class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-800 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm"
                                @click="open = false"
                                ref="cancelButtonRef"
                            >
                                Cancel
...
export default {
    setup() {
        ...
        const showing = ref("pending");

        onMounted(() => {
            axios.get(`/get-pending-orders`).then((res) => {
                if (res.status === 200) {
        ...
        function getpendingorders(page) {
            axios.get(`/get-pending-orders?page=${page}`).then((res) => {
                if (res.status === 200) {
                    orders.value = res.data.data;
                    last_page.value = res.data.last_page;
                }
        ...
        function getfailedorders(page) {
            axios.get(`/get-failed-orders?page=${page}`).then((res) => {
                if (res.status === 200) {
                    orders.value = res.data.data;
                    last_page.value = res.data.last_page;
                }
        ...
        function getsuccessorders(page) {
            axios.get(`/get-success-orders?page=${page}`).then((res) => {
                if (res.status === 200) {
                    orders.value = res.data.data;
                    last_page.value = res.data.last_page;
                }
        ...
        watch(current_page, (current_page, prevCurrent_page) => {
            ...
            if (showing.value == "pending") {
                getpendingorders(current_page);
            }
            if (showing.value == "failed") {
                getfailedorders(current_page);
            }
            if (showing.value == "success") {
                getsuccessorders(current_page);
            }
        });
        watch(showing, (showing, prevshowing) => {
            if (showing == "pending") {
                getpendingorders(1);
            }
            if (showing == "failed") {
