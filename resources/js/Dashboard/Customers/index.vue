...
export default {
  ...
  computed: {
    filteredcustomers() {
      ...
      if (this.filterData) {
        if (
          this.filterData.customersIds.length ||
          this.filterData.categoryIds.length
        ) {
          return customers.filter(
            (item) =>
              this.filterData.customersIds.includes(item.user_id) ||
  ...
  setup() {
    ...
    const query = ref("");
    onMounted(() => {
      axios.get(`get-customers?page=${current_page}`).then((res) => {
        if (res.status === 200) {
    ...
    function searchcustomers() {
      axios.get(`searchcustomers?query=${query.value}`).then((res) => {
        if (res.status === 200) {
          customers.value = res.data.data;
          last_page.value = res.data.last_page;
        }
    ...
    function getcustomers(page) {
      axios.get(`get-customers?page=${page}`).then((res) => {
        if (res.status === 200) {
          customers.value = res.data.data;
          last_page.value = res.data.last_page;
        }
